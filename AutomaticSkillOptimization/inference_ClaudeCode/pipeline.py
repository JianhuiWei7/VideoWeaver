...
from AutomaticSkillOptimization.args import (
    CLAUDE_ORIGINAL_CREATOR_PATH_EXPERT,
    CLAUDE_ORIGINAL_CREATOR_PATH_VANILLA,
    CLAUDE_WORKSPACE,
    DATASET_DIR,
    FAILED_DIR,
    HALF_STOPPED_DIR,
...
from AutomaticSkillOptimization.inference_ClaudeCode.claude_cli_tools import (
    active_processes,
...
from AutomaticSkillOptimization.inference_ClaudeCode.running_utils import (
    create_skill_task,
    done_gen_sessions,
    failed_gen_sessions,
    running_sessions,
...
from AutomaticSkillOptimization.prompts import (
    CODEX_EXPERT_SKILL_CREATOR_PREFIX as CLAUDE_EXPERT_SKILL_CREATOR_PREFIX,
    CODEX_TASK_PREFIX as CLAUDE_TASK_PREFIX,
    CODEX_TASK_PREFIX_WO_TASK_TRACKER as CLAUDE_TASK_PREFIX_WO_TASK_TRACKER,
    CODEX_VANILLA_SKILL_CREATOR_PREFIX as CLAUDE_VANILLA_SKILL_CREATOR_PREFIX,
    ...
    for attempt in range(task_max_retries):
        ...
        try:
            ...
            input_context = process_input(
                sub_dir_path,
                CLAUDE_TASK_PREFIX,
                CLAUDE_TASK_PREFIX_WO_TASK_TRACKER,
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Pipeline Run Claude Code Video Gen (inference only)")
    parser.add_argument("--dataset", type=str, default=DATASET_DIR, help="Path to the dataset directory")
    parser.add_argument("--max_concurrency", type=int, default=3, help="Maximum number of concurrent tasks")
    parser.add_argument("--timeout_seconds", type=int, default=10800, help="Execution timeout in seconds")
    parser.add_argument("--max_retries", type=int, default=6, help="Maximum retries for rate limit/system errors")
    parser.add_argument("--task_max_retries", type=int, default=3, help="Maximum retries for gen task")
    ...
    parser.add_argument("--exp_id", type=str, default="claude_inference", help="Experiment ID to group results")
    ...
    parser.add_argument("--claude_backend", type=str, choices=["deepseek", "seed", "opus"], default=os.environ.get("CLAUDE_BACKEND", "deepseek"), help="Claude Code backend")
    return parser.parse_args()


async def main(args):
    random.seed(args.random_seed)
    configure_claude_backend(args.claude_backend)
    setup_claude_workspace()
    fixed_skill_dir = os.path.join(CLAUDE_WORKSPACE, ".claude", "skills")
    ...
    if args.no_composition_skills:
        ...
        skill_creator_prefix = CLAUDE_VANILLA_SKILL_CREATOR_PREFIX
        original_creator_path = CLAUDE_ORIGINAL_CREATOR_PATH_VANILLA
    elif args.skill_type == "expert":
        skill_dir = fixed_skill_dir
        skill_creator_prefix = CLAUDE_EXPERT_SKILL_CREATOR_PREFIX
        original_creator_path = CLAUDE_ORIGINAL_CREATOR_PATH_EXPERT
    else:
    ...
    skill_dir_name = "claude_fixed_skills" if skill_dir else "none"
    ...
    if args.use_existing_skill_dir and args.existing_skill_dir:
        if os.path.exists(args.existing_skill_dir):
            if args.dry_run:
                print(f"Dry run: would sync composition skills from {args.existing_skill_dir} into fixed Claude skill dir {fixed_skill_dir}")
            else:
                sync_composition_skills_to_fixed_dir(
                    args.existing_skill_dir,
                    fixed_skill_dir,
                    dry_run=args.dry_run,
                    label="fixed Claude skill dir",
                )
                print(f"Synced composition skills from {args.existing_skill_dir} into fixed Claude skill dir {fixed_skill_dir}")
        else:
        ...
        if args.dry_run:
            print(f"Dry run: would restore archived composition skills from {args.new_skill_path} into fixed Claude skill dir {fixed_skill_dir}")
        else:
            sync_composition_skills_to_fixed_dir(
                args.new_skill_path,
                fixed_skill_dir,
            ...
            print(f"Restored archived composition skills from {args.new_skill_path} into fixed Claude skill dir {fixed_skill_dir}")
    elif args.no_composition_skills:
        clean_composition_skills_from_fixed_dir(fixed_skill_dir, dry_run=args.dry_run, label="fixed Claude skill dir")
    ...
    if args.dry_run:
        ...
        if args.no_composition_skills:
            ...
            print(f"Skill Strategy: Using {args.skill_type.upper()} skills via fixed Claude skill dir '{fixed_skill_dir}'.")
...
def cleanup(args):
    ...
    print("\nCaught KeyboardInterrupt. Stopping active Claude Code processes...")
...
if __name__ == "__main__":
    ...
    print(f"ClaudeCode cwd: {CLAUDE_WORKSPACE}")
    try:
